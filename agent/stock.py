...
async def chat_tool(
    ...
    """
        When user asks for something unrelated to the stock portfolio, you should use this tool to answer the question. The answers should be generic and should be relevant to the user query.
...
async def gather_stock_data(
    ...
    ctx.deps.state.be_stock_data = data["Close"]
    changes.append(JSONPatchOp(
        op="replace",
        path="/be_arguments",
        value={
...
async def allocate_cash(
    ...
    stock_data = ctx.deps.state.be_stock_data  # DataFrame: index=date, columns=tickers
